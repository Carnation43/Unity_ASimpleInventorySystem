...
namespace UnityEngine.UI.Extensions
{
    ...
    public class UIScrollToSelection : MonoBehaviour
    {
        ...
        {
            float viewTop = listAnchorPosition;
            float viewBottom = listAnchorPosition + maskLength;
            float itemTop = position;
            float itemBottom = position + targetLength;

            if (itemTop < viewTop)
            {
                // 物品在视口上方。返回一个正值以向上滚动。
                return viewTop - itemTop;
            }
            else if (itemBottom > viewBottom)
            {
                // 物品在视口下方。返回一个负值以向下滚动。
                return viewBottom - itemBottom;
            }
            return 0;
        }
