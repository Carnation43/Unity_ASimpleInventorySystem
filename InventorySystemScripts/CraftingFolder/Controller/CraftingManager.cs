...
/// </summary>
public class CraftingManager : MonoBehaviour, IResettable
{
    ...
    private void InitializeRecipeDictionary()
    {
        ...
        foreach (var recipe in allRecipes)
        {
            ...
            List<string> ingredientNames = recipe.ingredients
                .Select(ingredient => ingredient.item.name)
                .ToList();
    ...
    /// </summary>
    public void CheckForMatchingRecipe()
    {
    ...
    public bool AddItemToSlot(int slotIndex, InventorySlot itemSlot)
    {
        ...
        else if (_craftingSlots[slotIndex].item == itemSlot.item)
        {
    ...
    /// </summary>
    public void CraftItems(int amountToCraft)
    {
        ...
        {
            var slotToConsume = _craftingSlots.First(s => s != null && s.item.name == ingredient.item.name);
    ...
    public int CalculateMaxCraftableAmount()
    {
        ...
        foreach (var ingredient in MatchedRecipe.ingredients)
        {
            var slot = _craftingSlots.FirstOrDefault(s => s != null && s.item.name == ingredient.item.name);
    ...
    {
        if (_craftingSlots != null && _craftingSlots.Any(s => s != null))
        {
        ...
        Debug.Log("CraftingManager has been reset to default state.");
    }
}
