...
public class DetailsContent : MonoBehaviour, IResettableUI
{
    ...
    public void ToggleByInput()
    {
        ...
        GameObject selectedObj = selectionStateManager.LastItemSelected;
        if (selectedObj != null)
        {
            // decoupled by generic
            var slotUI = selectedObj.GetComponent<ISlotUI<InventorySlot>>();

            if (slotUI != null && slotUI.IData != null)
            {
                ShowPanel1_DetailsContent(slotUI.IData.item);
                return;
            }
        }
    }
    ...
    public void SetupItemDetails(Item item)
    {
        ...
        if (playAnimation)
        {
            ...
            if (SequenceController.instance != null)
            {
                SequenceController.instance.ResetUI();
                SequenceController.instance.StartPlaySequence();
            }
        }
        else
