...
public class InventoryManager : MonoBehaviour
{
    ...
    // InventoryEventChannel: The Broadcaster
    [Header("Broadcasting On")]
    [Tooltip("The event channel used to broadcast inventory changes to the UI.")]
    [SerializeField] private InventoryEventChannel _inventoryChannel;

    // The master list of all inventory slots. This is the single source of truth for what the player owns.
    public List<InventorySlot> inventory = new List<InventorySlot>();

    #region [Depleted codes] Change from the singleton pattern to the observer pattern
    ...
    public void TriggerInventoryUpdate()
    {
        ...
        {
            _inventoryChannel.RaiseInventoryUpdated();
        }
        else
        {
            Debug.LogWarning("[InventoryManager] Inventory Channel is missing !");
        }
    }
    ...
    public void AddItem(Item newItem)
    {
        ...
        // if not found or not stackable, create a new slot.
        inventory.Add(new InventorySlot(newItem));
        TriggerInventoryUpdate();     // trigger event
    }
    ...
    public void RemoveItem(InventorySlot slot)
    {
        ...
        // Passes detailed info (slot index, isDepleted) to the channel
        // so the UI can handle focus selection smarty after an item is consumed.
        if (_inventoryChannel != null)
        {
            _inventoryChannel.RaiseItemConsumed(slot, slotIsEmpty, slotIndex);
        }
    ...
    // Hepler method to get the total count of a specific item.
    // Used by the Crafting System and Recipe Book to check if the player has enough ingredients.
