...
public class CraftingView : MonoBehaviour
{
    ...
    private void OnDisable()
    {
        ...
        // Cleanup particles: Return active effects to the pool when the view is disabled
        if (animationParent != null && GameObjectPoolManager.Instance != null)
        {
            // includeInactive = true ensures that child object can be founded
            var particles = animationParent.GetComponentsInChildren<ItemParticleController>(true);

            foreach (var p in particles)
            {
                // retrieve active effects
                if (p.gameObject.activeSelf && p.sourcePrefab != null)
                {
                    GameObjectPoolManager.Instance.ReturnToPool(p.sourcePrefab, p.gameObject);
                }
            }
        }
    ...
    private void HandleConfirmStarted(InputAction.CallbackContext context)
    {
        ...
        if (InventoryController.instance.currentFocus != InventoryController.MenuFocus.Crafting) return;
