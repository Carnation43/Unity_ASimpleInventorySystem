...
/// </summary>
public class LevelUpView : MonoBehaviour, IResettableUI
{
    [Header("Data Source")]
    ...
    {
        if ((inputChannel != null && inputChannel.IsInputLocked) || !context.performed || InventoryController.instance == null || InventoryController.instance.currentFocus != InventoryController.MenuFocus.LevelUp)
        {
            return;
        }
        ...
        if (context.action.name == "NavigateRight")
        {
            ...
            if (_wallet.CurrentGlows >= totalPreviewCost + costForNextLevel)
            {
    ...
    {
        if ((inputChannel != null && inputChannel.IsInputLocked) || !context.started) return;
    ...
    public void HandleNavigateInput(InputAction.CallbackContext context)
    {
        ...
        if ((inputChannel != null && inputChannel.IsInputLocked) || !context.performed || InventoryController.instance == null || 
            InventoryController.instance.currentFocus != InventoryController.MenuFocus.LevelUp)
        {
            return;
        }
    ...
    private void UpdateAllUI()
    {
        ...
        currentGlowsText.text = _wallet.CurrentGlows.ToString();
    ...
    private void UpdateStatRow(AttributeRowUI row, int baseValue, int points, bool isSelected)
    {
        ...
        if (showSelected)
        {
            ...
            bool canAddPoint = _wallet.CurrentGlows >= totalPreviewCost + costForNextPoint;
